...
def new_pass(out: Path):
    ...
    data = {}
    data["pass_id"] = click.prompt("Pass ID", default=f"EVT-SINGLE_DAY-2026-{datetime.utcnow().strftime('%H%M%S')}")
    data["full_name"] = click.prompt("Full name")
    data["id_number"] = click.prompt("ID / Passport / Plate")
    data["phone"] = click.prompt("Phone", default="")
    data["operator"] = click.prompt("Event Organizer", default="Event Organizer")
    data["category"] = click.prompt("Category", type=click.Choice([c.value for c in PassCategory]), default="ATTENDEE")
    data["trip_type"] = click.prompt("Trip Type", type=click.Choice([t.value for t in TripType]), default="SINGLE_DAY")
    data["valid_from"] = click.prompt("Valid from (YYYY-MM-DD)", default=date.today().isoformat())
    data["valid_to"] = click.prompt("Valid to (YYYY-MM-DD)", default=(date.today().replace(day=date.today().day+3)).isoformat())
    data["gate"] = click.prompt("Gate", default="Main Gate")
    data["group_ref"] = click.prompt("Group / Reference", default="")
    data["issued_by"] = click.prompt("Issued by", default="GatePassX")
